...
WaitForXHR.prototype.command = function(
  ...
  api.execute(clientListen, [], function(res) {
    if (trigger) {
      ...
      else if (typeof trigger === "string") api.click(trigger);
    }
  });

  // console.log('Setting up timeout...');
  this.timeout = setTimeout(function() {
    command.api.execute(clientPoll, [], function({
      value: xhrs
    }: {
      value: ?Array<ListenedXHR>
    }) {
      //console.log('xhrss', xhrs);
      const matchingXhrs = xhrs
        ? xhrs.filter(xhr => xhr.url.match(command.urlPattern))
        : [];
      if (matchingXhrs) command.callback(matchingXhrs);
      else
        command.client.assertion(
          false,
          "Nothing heard",
          "XHR Request",
          `No XHR opened with pattern ${urlPattern} !`
        );
